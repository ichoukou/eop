...
public class AttentionMailServiceImpl<T extends AttentionMail> implements AttentionMailService<T> {
    ...
    @Override
    public int searchByMailNoAndCustomerIdsAndTime(String mailNo, List<String> customerId, String startTime, String endTime) {
    	Map map = new HashMap();
    	SimpleDateFormat simpleDate = new SimpleDateFormat("yyyy-MM-dd");
        if(startTime!=null && endTime!=null && !startTime.equals("") && !endTime.equals("")){
            try {
		...
		int list = dao.searchByMailNoAndCustomerIdsAndTime(map);
		return list;
    }
