...
public class CreateOrderTimerQuartz extends QuartzJobBean {
		...
		if (!isRunning) {
			...
			try {
				...
				if(!DateUtil.isVerifyDayInterval(lastEndDate,endDate)){
					...
					logger.error("跨天->检索开始时间："+strStarttime+"检索结束时间："+strEndtime);
				}
				...
				if(timerLog != null){timerLog = null;}
				isRunning = false;
			}
		}
	}
	...
	private BuildSearch setBuildSearch(Order order, MailObjectDTO traderinfo) {
		...
		if (StringUtils.isBlank(order.getMailNo())) {
			...
			if (order.getMailNo() != null) {
				...
				logger.info("createOrder:mailno=" + order.getMailNo());
			} else
		...
		if (order.getPartitionDate() != null) {
			...
			logger.info("createOrder:setPartitiondate=" + order.getPartitionDate());
		} else
			buildSearch.setPartitiondate(new Date());
		if (order.getAcceptTime() != null) {
			buildSearch.setAcceptTime(order.getAcceptTime());
			logger.info("createOrder:getAcceptTime=" + order.getAcceptTime());
		} else
		...
		if (traderinfo.getDistrict() != null) {
			...
			logger.info("createOrder:getDistrict=" + traderinfo.getDistrict());
		} else
			buildSearch.setDistrict("");
		
		
		logger.info("createOrder:buildSearch date end");
		return buildSearch;
	}
	...
	private boolean orderUpdate(JGOrderDTO jgOrderDTO) {
		...
		try {
			...
			flag = false;
			logger.error("金刚订单创建同步->面单绑定修改失败"+jgOrderDTO.getOrderLogisticsCode());
		}
		if (jgOrderDTO.getIsOffline() == 1) {
			...
			if(!jgWaybillUpdateService.addJgWaybillUpdate(jgWaybillUpdate)){
				logger.error("金刚订单创建同步->面单绑定进入JgWaybill表失败"+jgOrderDTO.getOrderLogisticsCode());
			};
		}
		return flag;
	}
