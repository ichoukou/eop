...
(function($) {
	...
	$.alerts = {
		...
		_show: function(title, msg, value, type, callback) {
			...
			if( $.alerts.dialogClass ) $j("#popup_container").addClass($.alerts.dialogClass);
			...
			switch( type ) {
				case 'alert':
					$j("#popup_message").after('<div id="popup_panel"><input type="button" value="' + $.alerts.okButton + '" id="popup_ok" /></div>');
				...
				break;
				case 'confirm':
					$j("#popup_message").after('<div id="popup_panel"><input type="button" value="' + $.alerts.okButton + '" id="popup_ok" /> <input type="button" value="' + $.alerts.cancelButton + '" id="popup_cancel" /></div>');
					$j("#popup_ok").click( function() {
						$.alerts._hide();
				...
				break;
				case 'prompt':
					$j("#popup_message").append('<br /><input type="text" size="30" id="popup_prompt" />').after('<div id="popup_panel"><input type="button" value="' + $.alerts.okButton + '" id="popup_ok" /> <input type="button" value="' + $.alerts.cancelButton + '" id="popup_cancel" /></div>');
		...
		_reposition: function() {
			var top = (($j(window).height() / 2) - ($j("#popup_container").outerHeight() / 2)) + $.alerts.verticalOffset;
			var left = (($j(window).width() / 2) - ($j("#popup_container").outerWidth() / 2)) + $.alerts.horizontalOffset;
			if( top < 0 ) top = 0;
			if( left < 0 ) left = 0;
			
			// IE6 fix
			if( $.browser.msie && parseInt($.browser.version) <= 6 ) top = top + $j(window).scrollTop();
		...
		_maintainPosition: function(status) {
			if( $.alerts.repositionOnResize ) {
				switch(status) {
					case true:
						$j(window).bind('resize', $.alerts._reposition);
					break;
					case false:
						$j(window).unbind('resize', $.alerts._reposition);
					break;
				}
			}
		}
