...
public class DateUtil {
	 ...
	 * return :结束日期
	 */
	public static Date getCireclDate(Date date,int month_num){
		GregorianCalendar gc = new GregorianCalendar();
		gc.setTime(date);
		gc.add(Calendar.MONTH, month_num);
	...
	public static boolean isSameDay(Date d1, Date d2) {
		return DateUtil.roundToDay1(d1).getTime() == DateUtil.roundToDay1(d2)
				.getTime();
	}
	...
	public static Date roundToDay(Date date) {
		...
		/*
		 * String hours=sf_date.substring(sf_date.indexOf(":")-2, sf_date.indexOf(":"));		
		if(StringUtils.equals("0", hours.substring(0,1))){
			hours=hours.substring(1);
		}Integer.parseInt(hours)*/
	
		
		try {
			gc.set(Calendar.HOUR_OF_DAY,sf.parse(sf_date).getHours());
		} catch (ParseException e) {
	...
	public static Date roundToHour(Date date) {
		...
		gc.set(Calendar.MINUTE, date.getMinutes());
		gc.set(Calendar.SECOND, date.getSeconds());
		gc.set(Calendar.MILLISECOND, 0);
		return gc.getTime();
	}
	...
	public static void main(String[] args) {
		// List list = listMonthOption(24);
		// for (int i = 0; i < list.size(); i++) {
		// Date d = (Date) list.get(i);
		// logger.debug(format(d, "yyyyMM"));
		// }
		logger.debug(format(new Date(), "yyyy-M-d h:m:s"));
	}
