...
public class DateUtil {
	 ...
	 * @return
	 */
	public static boolean isHourTimeDifference(Date beforeDate, Date afterDate,
			int nHour) {
	...
	/**
	 * 判断两个时间是否跨天，是隔天返回true
	 * 
	 * @param start
	 * @param end
	 * @return
	 */
	public static boolean isVerifyDayInterval(Date start, Date end) {
		if (!toDay(start).equals(toDay(end))) {
			return true;
		}
		return false;
	...
	public static String stringToDate(String str, int days)
			throws ParseException {
		SimpleDateFormat sdm = new SimpleDateFormat("yyyy-MM-dd");
		String result = null;
	...
	/**
	 * 北京时间东八区比国际标准时间早8小时，计算的时候需要在标准时间上加上八个小时
	 * 
	 * @param date
	 * @return
	 */
	private static Date next8Hour(Date date) {
		date = DateUtil.add(date, Calendar.HOUR, 8);
		return DateUtil.roundToHour(date);
	}
	...
	public static void main(String[] args) throws ParseException {
		// Date date = new Date();
		// System.out.println(getDateBefore(date, 15));

		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss");
		String s = "2012-7-1 13:24:30";
		String e = "2012-7-1 14:24:31";

		// double hourInterval = hourInterval(sdf.parse(s), sdf.parse(e));
		// System.out.println(hourInterval);
		// System.out.println(secondInterval(sdf.parse(s), sdf.parse(e)));
		// System.out.println(isHourTimeDifference(sdf.parse(s),
		// sdf.parse(e),1));
