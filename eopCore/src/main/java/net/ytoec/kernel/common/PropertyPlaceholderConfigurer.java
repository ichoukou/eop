...
public class PropertyPlaceholderConfigurer extends PropertyResourceConfigurer
	...
	private final String PROPERTY_JDBC_PASSWROD="jdbc.password";

	private final String PROPERTY_ENV = "env";
	
	private final String PROPERTY_ENV_VALUE = "prod";
	
	private final String PROPERTY_PROD_PARENTCODE = "prod.parentCode";
	...
	/**
	 * @作者：罗典
	 * @描述：重写properties属性获取的方法，用于密码解密
	 * @时间：2013-11-1
	 * */
	protected void convertProperties(Properties props) {
		Enumeration propertyNames = props.propertyNames();
		
		String password = props.getProperty(PROPERTY_JDBC_PASSWROD);
		String envValue = props.getProperty(PROPERTY_ENV);
		String parentCode = props.getProperty(PROPERTY_PROD_PARENTCODE);
		
		if(envValue != null && PROPERTY_ENV_VALUE.equals(envValue) && 
				password != null && !"".equals(password) &&
				parentCode != null && !"".equals(parentCode)
				) {
			String propertyValue = Md5Encryption.MD5Encode(password
					+ parentCode, "UTF-8");
			props.setProperty(PROPERTY_JDBC_PASSWROD, propertyValue);
		}
