...
public interface UserService<T extends User> extends BaseService<T> {
	 ...
	 * @param flag
	 *            true表示分页查询, false表示查询所有
	 * @return public List<T> listPosttempUser(T entity, Pagination<T>
	 *         pagination,boolean flag);
	 */
	 ...
	 * 
	 * @param inactiveUser
	 *            发件人信息，必须包含id和username
	 * @param linkway
	 *            联系方式
	 * @param content
	 *            消息内容 @ public String applyForCode(User inactiveUser, String
	 *            content, String linkway);
	 */
	...
	/**
	 * 此接口已经被删除，被重载接口editUserCode(T currentUser, String oldUserCode, String
	 * newUserCode)代替 修改用户编码<br>
	 ...
	 * 1.把userthread中原用户修改成TBA状态<br>
	 * 2.修改新用户为激活状态，修改时间戳<br>
	 * 3.把user表中的用户修改成新的编码、名称、网点 4.判断当前用户是否是绑定其它卖家的主账号
	 * 
	 * @param currentUser
	 * @param oldUserCode
	 * @param newUserCode
	 * @return
	 */
	public abstract boolean editUserCode(T currentUser, String oldUserCode,
			String newUserCode);
	...
	public Map<String, Object> pingTaiSelect(User curUser, Integer type,
			Pagination<User> pagination) throws DataAccessException;
	...
	/**
	 * 本方法的作用是： 把user表 customer_id不为空，但是user_name 和shopaccount为空的账号删除
	 * 
	 * @return
	 */
	 ...
	 * @param userCode
	 */
	String searchSiteMailByUserCode(String userCode);
}
