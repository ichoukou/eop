...
public class YtoUserServiceImpl<T extends User> implements YtoUserService<T> {

    private static final Logger              logger        = LoggerFactory.getLogger(YtoUserServiceImpl.class);
    // VIP用户激活判断编码状态的常量字段
    ...
    public boolean addUser(T entity) {
        ...
        /*
         * @2011-09-09/ChenRen
         * @Description 现在核心系统没有用户新增操作, 新增用户实际上是从 金刚 同步过来. 同步的用户只需要用户编码, 用户登录 核心系统 的时候要先根据 编码 激活才能登录. 所以这里[新增]取消验证.
         * 验证添加到[激活#activateUser]的方法里. // 在服务器进行用户帐号唯一性的验证 List<T> list = dao.searchUsersByName(entity); if (list.size()
         * > 0) { logger.error("新增失败![" + entity.getUserName() + "]已存在!"); } // 对密码进行MD5加密
        ...
        if (entity.getParentId() != null) {
            ...
            if (user == null) {
                logger.error("新增子账号失败. 父对象不存在. 参数信息[parentId:" + entity.getParentId() + "]");
                return false;
            }
            /*
            String customerId = user.getTaobaoEncodeKey();
            customerId += "," + entity.getTaobaoEncodeKey();
    ...
    public boolean delUserByUserName(String userName) {
        ...
        T entity = dao.getUserByUserName(userName);
        if (entity == null) {
            logger.error("entity is empity");
            return false;
        }
        //删除user信息,,删除的数据包括 user表的信息和user_custom表的信息
        boolean a = dao.delUser(entity);
        boolean b = ucDao.delUserCustom(userName);
        return true;
    }
    ...
    /*
     * public List<T> listPosttempUser(T entity, Pagination<T> pagination, boolean flag) { // field002里设置的是网点的Id
     * if(StringUtils.isEmpty(entity.getField002() ) ) { // 在action里指定了用户的网点Id，见 userAction.java#listPosttempUser //
     * 设置网点Id是为了根据网点Id去查询已经分配过的模板的用户 logger.error("参数异常! 没有指定用户的网点Id!"); return Collections.EMPTY_LIST; }
     * List<PosttempUser> puList = puDao.getPosttempUserByBranchId(Integer.parseInt(entity.getField002() ) ); List<T>
     * uList = searchUsers(entity, pagination, flag); for (PosttempUser posttempUser : puList) { for (T user : uList) {
     * if(user.getId().equals(posttempUser.getVipId() ) ) { // 设置用户的备用字段field003为false，标识当前用户已经分配过模板 //
     * 页面显示用户的时候判断该值，如果为false，不让网点选择该用户 user.setField003("false"); } } } // for posttempUser return uList; }
     */
    ...
    public T searchUserByUserNameAndPass(String userName, String password) {
        if (userName != "" && password != "") {
            ...
            if (dao.searchUserByNameAndPwd((T) user) != null && dao.searchUserByNameAndPwd((T) user).size() > 0) {
    ...
    public String checkUserCode(T entity) {
        ...
        // 编码不唯一
            ...
            msg = CHECK_USERCODE_NOTUNIQUEN + "<br> [userCode: " + entity.getUserCode() + "; size: " + size + "]";
        } else if (size == 1) {
            T e = list.get(0);
            // 编码已激活
    ...
    public boolean activateUser(T entity) {
        ...
        if (size == 0) {
            flag = false;
            logger.error(CHECK_USERCODE_NOTEXIST + "<br> [userCode: " + entity.getUserCode() + "]");
        }
        // 编码不唯一
        else if (size > 1) {
            flag = false;
            logger.error(CHECK_USERCODE_NOTUNIQUEN + "<br> [userCode: " + entity.getUserCode() + "; size: " + size
                         + "]");
        } else if (size == 1) {
            T e = list.get(0);
            // 编码已激活
            if (!"TBA".equalsIgnoreCase(e.getUserState())) {
                flag = false;
                logger.error(CHECK_USERCODE_ACTIVED + "<br> [userCode: " + entity.getUserCode() + "]");
            }
            // 如果编码未激活, 就激活用户
            if ("TBA".equalsIgnoreCase(e.getUserState())) {
                ...
                // 对密码进行MD5加密
                // 如果密码为空, 默认密码 yto1234
                String pwd = StringUtils.defaultIfEmpty(entity.getUserPassword(), "yto1234");
                entity.setUserPassword(Md5Encryption.MD5Encode(pwd));
                entity.setUserState("1"); // 用户状态原来是TBA, 激活后改称1, 正常.
                entity.setUserType(StringUtils.defaultIfEmpty(e.getUserType(), "1")); // 用户类型. VIP用户
        ...
        // 未知错误
            ...
            flag = false;
            logger.error(CHECK_USERCODE_UNKNOWERROE + "<br> [userCode: " + entity.getUserCode() + "]");
        }
    ...
    public boolean completeSite(T entity) {
        ...
        siteUser.setUserPassword(Md5Encryption.MD5Encode(entity.getUserPassword()));
        if(entity.getUserNameText()!=null)
        	siteUser.setUserNameText(entity.getUserNameText());
        if (entity.getMail() != null) siteUser.setMail(entity.getMail());
        if (entity.getMobilePhone() != null) siteUser.setMobilePhone(entity.getMobilePhone());
        if (entity.getTelePhone() != null) siteUser.setTelePhone(entity.getTelePhone());
        ...
        /*
         * @2012-01-09/ChenRen 　 ChenRen 10:02:17 现在网点激活还要不要自动生成子账号 圆通/李隆勇 10:02:35 不要了 // 新增子账号(客服帐号/财务帐号) if(flag) {
         * // 客服帐号 User kefu = new User(); kefu.setParentId(siteUser.getId()); kefu.setUserName(siteUser.getUserCode() +
         * "_kefu"); // 帐号规则：网点编码_kefu kefu.setUserPassword(Md5Encryption.MD5Encode(siteUser.getUserCode() + "_kefu") );
         * kefu.setUserNameText(siteUser.getUserNameText() + "_客服"); kefu.setUserType("21"); // 网点的客服类型
         * kefu.setUserState("1"); // 已激活 kefu.setField001(siteUser.getField001()); // 省份id // 财务帐号 User caiwu = new
         * User(); caiwu.setParentId(siteUser.getId()); caiwu.setUserName(siteUser.getUserCode() + "_caiwu"); //
         * 帐号规则：网点编码_kefu caiwu.setUserPassword(Md5Encryption.MD5Encode(siteUser.getUserCode() + "_caiwu") );
         * caiwu.setUserNameText(siteUser.getUserNameText() + "_财务"); caiwu.setUserType("22"); // 网点的客服类型
         * caiwu.setUserState("1"); // 已激活 caiwu.setField001(siteUser.getField001()); // 省份id dao.add((T) kefu);
         * dao.add((T) caiwu); }
         */
        return flag;
    }
    ...
    public boolean bindTaoBaoUserStep1(T entity) {
        ...
        // 淘宝用户
        if(StringUtils.isNotEmpty(entity.getUserSource()) && "1".equals(entity.getUserType())) {
	        List<T> list = dao.searchUsersByTaoBao(entity.getShopAccount());
	        int size = list.size();
	        if (size == 1) {
	            T e = list.get(0);
	            ...
	            flag = false;
	            logger.error("数据异常! 存在多个淘宝帐号相同的用户!" + " 参数信息[shopAccount:" + entity.getShopAccount() + "]");
	        } else if (size == 0) {
	            flag = false;
	            logger.error("数据异常! 指定用户不存在! " + " 参数信息[shopAccount:" + entity.getShopAccount() + "]");
	        }
        }
        // 平台用户的分仓账号
        else {
...
//        }
        ...
        if(flag) {
            //关联账号绑定同一网点
        	List<T> relatedUsers = getUsersByCustomerIds(entity2.getBindedCustomerId());
        	for(T relUser : relatedUsers){
        	    relUser.setUserState("1");// 用户状态原来是TBA, 激活后改称1, 正常.
        	    relUser.setSite(entity.getSite());
            ...
            Map subUserMap = this.getUserByParentId(entity.getId(), null); // null 表示不分页
//        	List<User> subUserList = (List) MapUtils.getObject(subUserMap, "listt", Collections.EMPTY_LIST);
        ...
        if (size == 1) {
            ...
            if ("TBA".equalsIgnoreCase(e.getUserState())) {
                e.setUserState("1");// 用户状态原来是TBA, 激活后改称1, 正常.
                e.setSite(entity.getSite());
                e.setUserCode(entity.getUserCode());
                e.setUpdateTime(new Date());
                e.setLoginTime(new Date());
                if ("9".equals(e.getField003())) {
                    e.setField003(null);
                }
    ...
    public T loginUserCheck(String userName, String password) {
        if (userName != "" && password != "") {
            ...
            if (list != null && list.size() > 0) {
                ...
                if("11".equals(userType) || "12".equals(userType) || "13".equals(userType)) {
    ...
    public T getSiteByVipId(Integer id) {
        ...
        if (list.size() > 1) {
            logger.error("数据异常! 当前用户存在多个网点数据! 参数信息[userId:" + id + "; lise.size():" + list.size() + "]");
        }
        return list.get(0);
    }
    ...
    public boolean bindTaoBaoAccount(T entity, String flag) {
        ...
        if (pUser == null) {
            logger.error("用户父对象不存在! 参数信息[user.id:" + entity.getId() + "; parentId:" + user.getParentId() + "]");
            return false;
        }
        ...
        // 取消绑定
            ...
            for (String customerId : pCustomerIdArr) {
                if (!StringUtils.isEmpty(customerId) && !customerId.equals(user.getTaobaoEncodeKey())) {
    ...
    public T getUserByCustomerId(String customerId) {
        ...
        if (TT.getParentId() != null) {
            ...
            if (pTT == null) {
                logger.error("用户的父对象不存在! 参数信息[userId:" + TT.getId() + ";parentId:" + TT.getParentId() + "]");
                return (T) new User();
            }
            return pTT;
        }
    ...
    public boolean bindSubAccount(T entity, String flag) {
        ...
        String state = "1".equals(flag) ? (StringUtils.isNotBlank(user.getUserCode()) ? "1" : "TBA") : "0";
        user.setUserState(state); // 用户状态：禁用(0)/已激活(1)

        /*boolean xx = dao.edit(pUser);
        if (xx) {
            xx = dao.edit(user);
        }*/
    ...
    public List<T> searchUsersByCodeTypeState(String userCode, String userType, String userState) {
        if (userCode == null || userType == null || userState == null) return null;
        if (userCode.trim().equals("") || userType.trim().equals("") || userState.trim().equals("")) return null;
        User user = new User();
    ...
    public boolean bindedAssociationAccount(T targetAccount, T associationAccount) {
        ...
        if (t == null && checkName(associationAccount)) { 
            ...
            //如果是新用户 同时也要在个性化配置表内插入一条账号自己的个性化配置信息
        ...
        result = dao.edit(targetAccount);
        if(result && t != null){
            associationAccount.setId(t.getId());
            dao.edit(associationAccount);
        }

        //主账号添加到userCustom表内
        UserCustom uc=new UserCustom();
        uc.setUserName(targetAccount.getUserName());
        uc.setBindedUserName(associationAccount.getUserName());
        uc.setType("1");
        uc.setCustomerId(associationAccount.getTaobaoEncodeKey());
        uc.setRelationalQuery("1");
        ucDao.add(uc);
        
        
        {// 同步绑定信息给子帐号
            List<T> subUsers = dao.getUserByParentId(targetAccount.getId());
            if (subUsers.isEmpty()) {
                return result;
            }
    ...
    public List<T> getUsersByCustomerIds(String customerIds) {
        ...
        String[] customerIdArray = StringUtils.split(customerIds, StringConstant.COMMA);
	...
	public boolean unbindUserCode(T entity) {
		...
		if(flag) {
		    //关联账号绑定同一网点
            List<T> relatedUsers = getUsersByCustomerIds(entity.getBindedCustomerId());
            for(T relUser : relatedUsers){
	...
	@Override
	public List<T> getAllUserMap(Map<String, Object> params,
			Pagination<T> pagination, boolean flag) {
		// TODO Auto-generated method stub
		if(params == null){
	    	params = new HashMap<String, Object>();
        ...
        return dao.getAllUserMap(params);
	}
	...
	public boolean initBranch(String userName) {
		...
		return dao.initBranch(userName);
	}
	@Override
