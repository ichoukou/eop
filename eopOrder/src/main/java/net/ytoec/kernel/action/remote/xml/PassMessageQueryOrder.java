...
public class PassMessageQueryOrder implements PassMessageObjectToXmlFragment {

	private static Logger logger = LoggerFactory
			.getLogger(PassMessageQueryOrder.class);
	...
	public PassMessageQueryOrder(Node order) {
		...
		for (int i = 0; i < nodeList.getLength(); i++) {
			...
			if (MAILNO.equals(node.getNodeName())) {
				...
				NodeList stepNode = stepsNode.item(stepsNode.getLength() - 1)
						.getChildNodes();
				...
				for (int j = 0; j < stepNode.getLength(); j++) {
					...
					if (ACCEPTTIME.equals(stepEle.getNodeName())) {
						...
						Element ele = (Element) stepEle;
						if (ele.getFirstChild() != null)
							step.setStatus(ele.getFirstChild().getNodeValue()
									.trim());
					} else if (REMARK.equals(stepEle.getNodeName())) {
						Element ele = (Element) stepEle;
						if (ele.getFirstChild() != null)
							step.setRemark(ele.getFirstChild().getNodeValue()
									.trim());
						...
						Element ele = (Element) stepEle;
						if (ele.getFirstChild() != null) {
							step.setSiteCode(ele.getFirstChild().getNodeValue()
									.trim());
							/**
							 * 此处同时给contactWay赋值
							 */
