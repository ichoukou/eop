...
//		User currUser = (User) session.get(Constants.SESSION_USER);
		...
		contractList = userThreadContractService.getContractersByUserNameAndType(currUser.getUserName(),currUser.getUserType());
		if(contractList==null || contractList.size() == 0){
			if(currUser.getParentId() != null){//承包区，客服，财务没有分配客户的时候
				userBeanList = null;
				pagination.setTotalRecords(0);
			}else{							   //网点账号
				userBeanList = userBeanService.getUserList(null,currUser.getSite(),
						userCode, userName, userState, pagination, true);
				pagination.setTotalRecords(userBeanService.countUserList(
						null,currUser.getSite(), userCode, userName, userState, pagination, false));
			}
			...
			userBeanList = userBeanService.getContractUserList(null,ids,pagination,userState,true);
			pagination.setTotalRecords(userBeanService.countContractUserList(null,ids,pagination,userState,false));
		}
...
//		User currUser = (User) session.get(Constants.SESSION_USER);
	...
	public String listPosttempUser() throws Exception {
		...
		if(contractList != null && contractList.size() != 0){ //四种账号类型分配客户的子账户(客服，财务，客服、财务，承包区)的选择用户
			...
			//直客表的客户数
			userThreadList = userThreadService.getContractUserList("0",ids, pagination,"1",true);
			...
			pagination.setTotalRecords(userBeanService.countContractUserList("0",ids,pagination,"1",false));
			...
			// 网点账号的选择用户
			List<UserThreadContract> siteContractList = userThreadContractService.searchContractsBysiteId(currUser.getId()+"","2");
			ArrayList<Integer> ids = new ArrayList<Integer>();
	   		...
	   		List<UserThread> utLists = listPosttempUserThread(user, pagination, false);
...
//		User currUser = (User) session.get(Constants.SESSION_USER);
		...
		for(String cur : Resource.getBindedCustomerIdList(currUser)){
			...
			if(u!=null){
				if(StringUtils.isNotEmpty(u.getShopName())) bindUserList.add(u);
			}
		}
...
//		User currUser = (User) session.get(Constants.SESSION_USER);
		...
		try {
			...
			if(isSuccess && readCookie("password") != null && "1".equals(readCookie("isRemeber").toString())){
...
//		User currUser = (User) session.get(Constants.SESSION_USER);
		if(StringUtils.isNotBlank(oldPwd)){
		    ...
		    isValidate = isSuccess ? "true" : "false";
		    putMsg(JsonResponse.INFO_TYPE_MSG, isSuccess, isSuccess ? "" : "原密码不正确", "");
		}
		else{
	...
	public String checkCustomerId(){
		if(user == null || StringUtils.isBlank(user.getTaobaoEncodeKey())){
			...
			isValidate = (user != null && user.getId() != null) ? "false" : "true";
		}
		putMsg(JsonResponse.INFO_TYPE_MSG, StringUtils.equals(isValidate, "true"), "", "");
		return "jsonRes";
	}
	...
	public String cancelRelation(){
		...
		if(list!=null && list.size()>0) {
			for(Integer id : list) {
				...
				boolean result = userRelationService.cancelRelation(currentUser, user);
				if (result) {
	...
	public void updateUserStateOfUserThread() {
		...
		if (users != null && users.size() > 1) {
			...
			if (userThreads != null && userThreads.size() > 0) {
				...
				if (StringUtils.isNotEmpty(userThread.getUserCode()) && StringUtils.startsWith(userThread.getUserCode(), "YT")) {
...
//			User cu = (User) session.get(Constants.SESSION_USER);
			...
			isValidate = user.getUserCode().equals(cu.getUserCode() ) ? "true" : "false";
		}
	...
	public String checkNewUserCode() throws Exception {
		if (user != null && StringUtils.isNotBlank(user.getUserCode())) {
		    ...
		    if(list.size() > 0){
		        ...
		        ajaxAlertText = dtoBranch == null ? "网点不存在" : StringUtils.defaultIfBlank(dtoBranch.getText(), "名称未填写");
		        putMsg(JsonResponse.INFO_TYPE_SUCCESS, dtoBranch != null, ajaxAlertText, "");
		    }
		    else{
	 ...
	 * 修改用户编码<br>
	 * 1.修改user表中的数据
	 * 2.把userthread中的老数据修改成tba，新数据改成1
	 * 3.刷新session
	 */
...
//			User cu = (User) session.get(Constants.SESSION_USER);
			...
			if(!cu.getUserCode().equals(user.getUserCode() )) {
			    // 修改数据
				boolean flag = userService.editUserCode(cu,cu.getUserCode(), user.getUserCode());
...
//					user.setUserCode(user.getUserCode());
					...
					//同时修改关联账户的网点编码
					for(User u : uList){
					    userService.editUserCode(cu,u.getUserCode(), user.getUserCode());
					}
				    ...
				    putMsg(JsonResponse.INFO_TYPE_ERROR,false,"数据更新失败，请重新操作！","");
				}
			}
			else{
			    putMsg(JsonResponse.INFO_TYPE_ERROR,false,"新用户编码和原用户编码相同，请修改后重新操作！","");
			}
		}
	...
	public List<UserThread> listPosttempUserThread(User entriy,
		...
		if(entriy.getIds()!=null && entriy.getIds().size() != 0){
			uList = userThreadService
			.searchUsersBySiteCodeAndNameAndNotId("0",entriy.getIds(),entriy.getSite(), null, "1",
					entriy.getUserName(), pagination, flag);
		}else{
			uList = userThreadService
			.searchUsersBySiteCodeAndName("0",entriy.getSite(), null, "1",
					entriy.getUserName(), pagination, flag);
		}
...
//		    if(StringUtils.equals("2", tempUser.getUserType())){
		        List<UserThreadContract> utcList = userThreadContractService.getContractersByUserNameAndType(tempUser.getUserName(),null);
		        if(utcList != null && utcList.size() > 0){
		            List<UserThread> userThreadList = new ArrayList<UserThread>();
		            for(UserThreadContract utc : utcList){
		                UserThread tempUT = userThreadService.getUserById(utc.getConractAreaId());
		                if(tempUT != null){
...
//		    }
		...
		pagination.setTotalRecords(Integer.parseInt( map.get("totalRecords").toString() ) );
...
//		User currUser = (User) super.session.get(Constants.SESSION_USER);
	...
	public String addSubAccount() throws Exception {
		...
		if (!flag) {
			isValidate = "name";
			return "toAddSubAccount";
		} else if(StringUtils.isEmpty(user.getUserType()) || user.getUserType().length() > 2){
		    isValidate = "userType";
            return "toAddSubAccount";
		}
		...
		//平台用户添加分仓或入驻企业
		if("1".equals(user.getUserType()) && "4".equals(currUser.getUserType()) && StringUtils.isNotBlank(user.getChildType())){
		...
		if(ids != null && !"".equals(ids) && "2".equals(currUser.getUserType())){ 
			User currentUser = super.readCookieUser();
			List<UserThreadContract> userContractList = new ArrayList<UserThreadContract>();
			String[] idss = ids.split(",");
			for(int s=0; s<idss.length; s++){
				UserThread userth = userThreadService.getUserById(Integer.parseInt(idss[s]));
		...
		if("true".equalsIgnoreCase(isValidate)) {
        	...
        	if("11".equals(userType) || "12".equals(userType) || "13".equals(userType)|| "1".equals(userType)) {
        		// 卖家要在个性化配置表里增加数据
        		...
        		if("1".equals(userType) && "4".equals(currUser.getUserType()) && list.size() == 0){
        			userCustom.setUserName(user.getUserName());
        			userCustomService.add(userCustom);
        		}
        	}
		}
		if("4".equals(currUser.getUserType()) && StringUtils.isNotBlank(user.getChildType())){
			if("true".equalsIgnoreCase(isValidate)) {
	...
	public void deleteCustomersList(){
		...
		if(currUser.getParentId() == null){
			userThreadContractService.deleteContractByConractAreaId(null,conractAreaId,site);
		}else {
			userThreadContractService.deleteContractByConractAreaId(addUserName,conractAreaId,site);
		}
	...
	public String toEditSubAccount() {
		...
		List<UserThreadContract> utcList = userThreadContractService.getContractersByUserNameAndType(user.getUserName(),null);
		if(!"2".equals(user.getUserType()) && (utcList == null || (utcList !=null && utcList.size() == 0))){
...
//		User currUser = (User) session.get(Constants.SESSION_USER);
		 ...
		 * 当网点创建的一个非承包区子账号（没有分配客户）进行修改账号类型为“承包区”，
		 * 需要修改子账号的site，usercode字段为子账号名称
		 * */
	...
	/**
     * 未绑定客户编码的卖家，根据最近的10条订单的运单号去金刚接口取网点编码
     * 1.根据当前卖家customerId取最近10条订单
     * 2.拿这10个运单号去金刚接口取网点编码，然后过滤出最匹配的网点信息集合
     * 3.返回网点信息给弹出层使用。
     */
    public String getSiteInfo(){
      ...
      //单个运单号获取网点
        if(StringUtils.isNotEmpty(mailNo)){
            ...
            List<BranchMailNoDTO> list = questionnaireService.searchOrgCodeByMailNo(mailNoArray);
            if(list != null && list.size() > 0){
                String siteCode = list.get(0).getOrgCode();
                
                if(Resource.dtoBranchMap == null || Resource.dtoBranchMap.isEmpty()){
                    Resource.UpdateDotCache();
                }
                
                DtoBranch branch = Resource.dtoBranchMap.get(siteCode);
                
                putMsg(JsonResponse.INFO_TYPE_SUCCESS, siteCode != null && branch != null, siteCode, siteCode != null && branch != null ? branch.getText() : "您输入的运单号没有对应网点，请检查后重新操作！");
            }
            else{
                putMsg(JsonResponse.INFO_TYPE_ERROR, false, "您输入的运单号没有对应网点，请检查后重新操作！", "");
            }
        }
        //多运单号获取网点
            ...
            params.put("limit", 10);
            List<String> mailNos = orderService.queryTenOrdersByCustomerId(params);
            List<BranchMailNoDTO> list = questionnaireService.searchOrgCodeByMailNo(mailNos.toArray(new String[0]));
            if(list != null && list.size() > 0){
                //过滤掉重复的网点编码
                ...
                for(String siteCode : siteCodeList){
                    if(Resource.dtoBranchMap.get(siteCode) != null){
                        branchList.add(Resource.dtoBranchMap.get(siteCode));
                    }
                }
                putMsg(JsonResponse.INFO_TYPE_MSG, branchList.size() > 0, "", "",branchList);
            }
            else{
                putMsg(JsonResponse.INFO_TYPE_ERROR, false, "您尚无运单记录，无法自动获取用户编码，请向所属网点申请！", "");
            }
        }
        else{
    ...
    public String generateUserCode(){
        ...
        else if(StringUtils.equals(user.getUserCode(), "autoGen") && StringUtils.isNotEmpty(user.getUserName())){
            UserThread ut = new UserThread();
            ut.setUserName(user.getUserName());
            ut.setSiteCode(user.getSite());
            ut = userThreadService.generateUserThread(ut);
            putMsg(JsonResponse.INFO_TYPE_SUCCESS,true,ut.getUserCode(),ut.getId().toString());
        }
        else{
    ...
    public String delUserCode(){
        if(userThreadId != null && userThreadId > 0){
            UserThread ut = userThreadService.getUserById(userThreadId);
            if(ut != null && 
                    StringUtils.isNotEmpty(ut.getUserCode()) && 
                ...
                putMsg(JsonResponse.INFO_TYPE_ERROR,false,"该客户已经被绑定或数据有误不能删除，操作失败","");
            }
        }
        else{
...
//			List<UserThreadContract> contractList = userThreadContractService.searchContractByConractAreaId(u.getId(), "2");
//			if(contractList!=null && !contractList.isEmpty()){//被分配给承包区了
//				User cbq = userService.getUserByUserName(contractList.get(0).getAddUserName());
//				String site = cbq.getSite();
//				DtoBranch dtoBranch = Resource.getDtoBranchByCode(site);
//				ajaxAlertText = dtoBranch == null ? "网点不存在" : dtoBranch.getText();
//			}else{
...
//			}
			if(u != null && StringUtils.isNotEmpty(u.getSiteCode())){
			    DtoBranch dtoBranch = Resource.getDtoBranchByCode(u.getSiteCode());
			    ajaxAlertText = dtoBranch == null ? "网点不存在" : dtoBranch.getText();
			}
			else{
...
//		cu.setUserState("TBA");//设为未激活状态
    ...
    public String addCustomersList(){
		...
		if(currentUser.getParentId() != null){
			//承包区子账号进来只能看到被分配的直客
			usedCustomers = userThreadContractService.getContractersByUserNameAndType(currentUser.getUserName(), "2");
			if(usedCustomers != null && usedCustomers.size() != 0){
				for(int i=0; i<usedCustomers.size(); i++){
					UserThread ut = userThreadService.getUserById(usedCustomers.get(i).getConractAreaId());
					
					List<UserThreadContract> utcs = userThreadContractDao.searchContractByConractAreaId(usedCustomers.get(i).getConractAreaId(),null);
			...
			return "toAddSubAccountJSON";
		}else {
			//所有客户
			customersList = userThreadService.searchAllUsersBySite(currentUser.getSite());
			
			//该用户已经承包出去的客户
			usedCustomers = userThreadContractService.searchContractsBysiteId(String.valueOf(currentUser.getId()),null);
		}
		...
		for(int i=0; i<customersList.size(); i++){
			...
			for(int j=0; j<usedCustomers.size(); j++){
				...
				if(user.getId().equals(utc.getConractAreaId())){
					...
					List<UserThreadContract> utcList2 = userThreadContractService.searchContractByConractAreaId(user.getId(),"2");
					if(utcList2 != null && utcList2.size() == 1){
						customersList.get(i).setIsAgain("no");
					}
					
					//模板客户的分配---已经被分配过了
					posttempUserList = posttempService.getPosttempUserByBranchIdAndVipId(branchId, user.getId());
		...
		if(currentUser.getParentId() != null){
			...
			if(usedCustomers != null && usedCustomers.size() != 0){
				for(int i=0; i<usedCustomers.size(); i++){
					...
					UserThread ut = userThreadService.getUserById(utc.getConractAreaId());
					
					List<UserThreadContract> utcs = userThreadContractDao.searchContractByConractAreaId(utc.getConractAreaId(),null);
					if(utcs != null && utcs.size() >= 2){
						ut.setContractStatus("true");  //已经分配的客户
						
						//查询承包区表里是否存在当前用户的的当前客户，如果是用来显示为勾选且活性
						List<UserThreadContract> utcList = userThreadContractService.searchContractByConractAreaIdAndUserName(utc.getConractAreaId(),editUserName);
		...
		for(int i=0; customersList!=null && i<customersList.size(); i++){
			...
			for(int j=0; j<usedCustomers.size(); j++){
				...
				if(user.getId().equals(utc.getConractAreaId())){
					...
					//查询承包区表里是否存在当前用户的的当前客户，如果是用来显示为勾选且活性
					List<UserThreadContract> utcList = userThreadContractService.searchContractByConractAreaIdAndUserName(user.getId(),editUserName);
					if(utcList != null && utcList.size() == 1){
	...
	public String eidtMobile(){
		User currentUser = super.readCookieUser();
		logger.info("id="+currentUser.getId()+"=userName="+currentUser.getUserName());
	...
	public String eidtUserName(){
		...
		List<UserThread> userThreadList = userThreadService.searchUsersByCode(ut);
