...
public class MonitorServiceImpl<T extends MonitorBean> implements
					...
					for (EccoreSearchResultDTO eccore : eccoreSearchList) {
						...
						if (eccore.getMailNo() != null) {
							...
							if(userClassify==1) {//卖家
								List<AttentionMail> list = attenService.searchByMailNoAndCustomerIdsAndTime(eccore.getMailNo(), bindedId, startTime, endTime);
								if (list != null && list.size() > 0)
									monitor.setIsAttention(1);
							} else if(userClassify==2 && userId!=null) {//网点
								List<String> customerIds = new ArrayList<String>();
								customerIds.add(userId.toString());
								List<AttentionMail> list = attenService.searchByMailNoAndCustomerIdsAndTime(eccore.getMailNo(), customerIds, startTime, endTime);
								if (list != null && list.size() > 0)
									monitor.setIsAttention(1);
							}
						}
	 ...
	 * @return
	 */
	private List<T> getAttentionList(int userClassify, Integer userId, String startTime, String endTime, Integer numProv, Integer numCity, Integer numDistrict,
			String mailNO, String buyerName, String buyerPhone,
	...
	public List<T> getSiteMonitorList(Integer tabFlag, Integer status,
		...
		} else if(tabFlag == 2) {//网点关注运单(关注订单只显示最多50条)
			return getAttentionList(2, userId, startTime, endTime, numProv, numCity, numDistrict, mailNoOrderType, buyerName, buyerPhone, 2, pagination, bindedId);
		} else{
			return new ArrayList();
		}
	}
