		...
		var form = function() {
			...
			$('#addCustomers').live('click',function(ev){
				...
				if(els.pageHid.val() == 'add'){
					...
					$.ajax({
							url : 'user!addCustomersList.action?r='+ Math.random(),
							type:'post',
							async: false,
							cache:false,
							dataType:'json',
							success : function(data) {
								...
								if(data.noDeliveryList != '[]' && data.noDeliveryList.length > 0){
									$('#smallWindow tbody').append("<tr style='height:30px;'><td colspan='2'><b>还未分配的客户有：</b></td></tr>");
								}
								for(var i=0, len=data.noDeliveryList.length; i<len; i++) {
									if(i%2 == 0){
										index = i/2;
										$('#smallWindow tbody').append("<tr class='trNoID"+index+"' style='height:30px;'></tr>");
									}
									
									$('.trNoID'+index).append("<td><span><input name='customer' id='customer"+data.noDeliveryList[i].id+"' type='checkbox' value='"+data.noDeliveryList[i].id+ "' /></span><span class='tabtop_span'>"+data.noDeliveryList[i].userName +"</span></td>");
									
									//记录刚才选中的
									var customersHid = document.getElementsByName('customerHid');
								...
								for(var i=0, len=data.yesDeliveryList.length; i<len; i++) {
									if(i%2 == 0){
										index = i/2;
									...
									if(data.yesDeliveryList[i].contractStatus == 'true'){
										if(userType == '2' || data.yesDeliveryList[i].isAgain == 'no'){
											...
											$('.trYesID'+index).append("<td><span><input name='customer' class='yesDelivery "+data.yesDeliveryList[i].userName+"' id='customer"+data.yesDeliveryList[i].id+"' type='checkbox' value='"+data.yesDeliveryList[i].id+ "' /></span><span class='tabtop_span'>"+data.yesDeliveryList[i].userName+"</span></td>");
										}
									}
									else if(data.yesDeliveryList[i].mailNoStatus == 'true'){
										if($('#lj3').is(':checked') != true){
											...
											$('.trYesID'+index).append("<td><span><input disabled=true checked=true id='customer"+data.yesDeliveryList[i].id+"' type='checkbox' value='"+data.yesDeliveryList[i].id+ "' /></span><span class='tabtop_span'>"+data.yesDeliveryList[i].userName+"</span></td>");
										}
									}
									else{
										$('.trYesID'+index).append("<td><span><input name='customer' class='yesDelivery "+data.yesDeliveryList[i].userName+"' id='customer"+data.yesDeliveryList[i].id+"' type='checkbox' value='"+data.yesDeliveryList[i].id+ "' /></span><span class='tabtop_span'>"+data.yesDeliveryList[i].userName +"</span></td>");
										...
										for(var i=0; i<customers.length; i++){
											if(customers[i].checked == true){
												if($('#customerHid'+customers[i].value).length == 0){
													if(customers[i].disabled == true){
														els.newAccForm.append("<input type='checkbox' style='display:none;' disabled=true name='customerHid' id='customerHid"+customers[i].value+"' value='"+customers[i].value+"' checked=true />");
													}else{
														var strs = customers[i].className.split(' ');
														if(strs[0] == 'yesDelivery'){
															els.newAccForm.append("<input type='checkbox' class='"+customers[i].className+"' style='display:none;' name='customerHid' id='customerHid"+customers[i].value+"' value='"+customers[i].value+"' checked=true />");
														}
														else if(strs[0] == 'isContractBox'){//勾选了运费模板用户
															els.newAccForm.append("<input type='checkbox' class='"+customers[i].className+"' style='display:none;' name='customerHid' id='customerHid"+customers[i].value+"' value='"+customers[i].value+"' checked=true />");
														}else{
															els.newAccForm.append("<input type='checkbox' style='display:none;' name='customerHid' id='customerHid"+customers[i].value+"' value='"+customers[i].value+"' checked=true />");
														}
													}
												}
											}else{
												if($('#customerHid'+customers[i].value).val() != 'undefined'){
													$('#customerHid'+customers[i].value).remove();
												}
