//====================================================================================================
//  [插件名称] jQuery formValidator Beta
//----------------------------------------------------------------------------------------------------
...
{
	//全局配置
	initConfigBeta : function(controlOptions)
	{
		var settings = {};
		...
		if(settings.formID!=""){
		    $("#"+settings.formID).submit(function(){
		        if(settings.ajaxForm!=null){
		            $.formValidatorBeta.ajaxForm(settings.validatorGroup,settings.ajaxForm);
			        return false;
				}
		        else{
					return $.formValidatorBeta.bindSubmit(settings);
                }
            });
		}
		...
		if (!window.onShowHtml) {
			$.ajax({async:false,type: "GET",url: themedir + "themes/"+settings.theme+"/js/theme.js",dataType: "script",
				error :function(){
					...
					console.log('当前皮肤加载出错，请确认皮肤【'+settings.theme+'】是否存在');
				}
			}); 
		}
		//读取主题对应的样式
	...
	bindSubmit : function(settings)
	{
		...
		return $.formValidatorBeta.pageIsValid(settings.validatorGroup);
	},
	...
	sustainType : function(elem,validateType)
	{
		...
		switch(validateType)
		{
			case "formValidatorBeta":
				return true;
			case "inputValidatorBeta":
				return (srcTag == "INPUT" || srcTag == "TEXTAREA" || srcTag == "SELECT");
			case "compareValidatorBeta":
				return ((srcTag == "INPUT" || srcTag == "TEXTAREA") ? (stype != "checkbox" && stype != "radio") : false);
			case "ajaxValidatorBeta":
				return (stype == "text" || stype == "textarea" || stype == "file" || stype == "password" || stype == "select-one");
			case "regexValidatorBeta":
				return ((srcTag == "INPUT" || srcTag == "TEXTAREA") ? (stype != "checkbox" && stype != "radio") : false);
			case "functionValidatorBeta":
			    return true;
			case "passwordValidatorBeta":
	...
	appendValid : function(id, setting )
	{
		...
		if(!$.formValidatorBeta.sustainType(elem,validateType)) return -1;
		//重新初始化
		if (validateType=="formValidatorBeta" || elem.settings == undefined ){elem.settings = new Array();}   
	...
	{
		var initConfigBeta = $.formValidatorBeta.getInitConfig(elem.validatorGroup);
		...
		{
			if(initConfigBeta.mode == "SingleTip")
			{
				//显示和保存提示信息
			...
			else
			{
				...
				else
				{
				    if(elem.validatorPasswordIndex > 0 && showclass =="onCorrect"){
				        ...
				        var level = $.formValidatorBeta.passwordValid(elem);
						showmsg = ""
						...
						if(showmsg!="")
						{
							$.formValidatorBeta.setTipState(elem,'onError',showmsg);
							return;
						}
	...
	resetTipState : function(validatorGroup)
	{
		...
		var initConfigBeta = $.formValidatorBeta.getInitConfig(validatorGroup);
		$.each(initConfigBeta.validObjects,function(){
			var setting = this.settings[0];
			var passed = setting.defaultPassed;
			$.formValidatorBeta.setTipState(this, passed ? "onCorrect" : "onShow", passed ? $.formValidatorBeta.getStatusText(this,setting.onCorrect) : setting.onShow );	
		});
	},
	...
	{
		$.formValidatorBeta.setTipState($("#"+tipID).get(0), "onError", showmsg);
	},
	...
	showMessage : function(returnObj)
	{
		...
		var intiConfig = $.formValidatorBeta.getInitConfig(elem.validatorGroup);
		if (!isValid)
		{		
			showclass = "onError";
			if(setting.validateType=="ajaxValidatorBeta")
			{
				...
				{
				    showmsg = $.formValidatorBeta.getStatusText(elem,setting.onError);
				}
			}
			else
			{
				showmsg = (returnObj.errormsg==""? $.formValidatorBeta.getStatusText(elem,setting.onError) : returnObj.errormsg);
			...
			{
				$.formValidatorBeta.setTipState(elem,showclass,showmsg);
			}
		}
		else
		{		
			//验证成功后,如果没有设置成功提示信息,则给出默认提示,否则给出自定义提示;允许为空,值为空的提示
			showmsg = $.formValidatorBeta.isEmpty(id) ? setting.onEmpty : $.formValidatorBeta.getStatusText(elem,setting.onCorrect);
			$.formValidatorBeta.setTipState(elem,"onCorrect",showmsg);
		}
		return showmsg;
	},
	...
	showAjaxMessage : function(returnObj)
	{
		...
		{
			$.formValidatorBeta.ajaxValid(returnObj);
		}
		else
		{
			if(setting.isValid!=undefined && !setting.isValid){
				elem.lastshowclass = "onError"; 
				elem.lastshowmsg = $.formValidatorBeta.getStatusText(elem,setting.onError);
			}
			$.formValidatorBeta.setTipState(elem,elem.lastshowclass,elem.lastshowmsg);
		}
	},
    ...
    isEmpty : function(id)
    {
        return ($("#"+id).get(0).settings[0].empty && $.formValidatorBeta.getLength(id)==0);
    },
    
	//对外调用：判断单个表单元素是否验证通过，不带回调函数
    isOneValid : function(id)
    {
	    return $.formValidatorBeta.oneIsValid(id).isValid;
    },
	...
	oneIsValid : function (id)
	{
		...
		var elem = $("#"+id).get(0);
		var initConfigBeta = $.formValidatorBeta.getInitConfig(elem.validatorGroup);
		returnObj.initConfigBeta = initConfigBeta;
		returnObj.id = id;
		returnObj.ajax = -1;
		...
		$.formValidatorBeta.resetInputValue(true,initConfigBeta,id);
		for ( var i = 0 ; i < settingslen ; i ++ )
		{   
			if(i==0){
				//如果为空，直接返回正确
				if($.formValidatorBeta.isEmpty(id)){
			...
			{
				case "inputValidatorBeta":
					$.formValidatorBeta.inputValid(returnObj);
					break;
				case "compareValidatorBeta":
					$.formValidatorBeta.compareValid(returnObj);
					break;
				case "regexValidatorBeta":
					$.formValidatorBeta.regexValid(returnObj);
					break;
				case "functionValidatorBeta":
					$.formValidatorBeta.functionValid(returnObj);
					break;
				case "ajaxValidatorBeta":
			...
			if(!settings[i].isValid) {
				...
				returnObj.isValid = true;
				returnObj.setting = settings[0];
				if (settings[i].validateType == "ajaxValidatorBeta"){break};
			}
		}
		$.formValidatorBeta.resetInputValue(false,initConfigBeta,id);
		return returnObj;
	},

	//验证所有需要验证的对象，并返回是否验证成功（如果曾经触发过ajaxValidatorBeta，提交的时候就不触发校验，直接读取结果）
	pageIsValid : function (validatorGroup)
	{
		...
		var initConfigBeta = $.formValidatorBeta.getInitConfig(validatorGroup);
		initConfigBeta.status = "sumbiting";
		initConfigBeta.ajaxCountSubmit = 0;
		//遍历所有要校验的控件,如果存在ajaxValidatorBeta就先直接触发
		$.each(initConfigBeta.validObjects,function()
		{
			if($(this).length==0){return true};
			if (this.settings[0].bind && this.validatorAjaxIndex!=undefined && this.onceValided == undefined) {
				returnObj = $.formValidatorBeta.oneIsValid(this.id);
				if (returnObj.ajax == this.validatorAjaxIndex) {
					initConfigBeta.status = "sumbitingWithAjax";
					$.formValidatorBeta.ajaxValid(returnObj);
				}
			}
		});
		//如果有提交的时候有触发ajaxValidatorBeta，所有的处理都放在ajax里处理
		if(initConfigBeta.ajaxCountSubmit > 0){return false}
		//遍历所有要校验的控件
		$.each(initConfigBeta.validObjects,function()
		{
			//只校验绑定的控件
				...
				if (name_list.indexOf("^"+name+"^") == -1) {
					...
					if (returnObj) {
						...
						if (!returnObj.isValid) {
							//记录不含ajaxValidatorBeta校验函数的校验结果
							isValid = false;
						...
						if (initConfigBeta.mode != "AlertTip") {
							var tipID = this.settings[0].tipID;
							if (error_tip.indexOf("^" + tipID + "^") == -1) {
								if (!returnObj.isValid) {error_tip = error_tip + tipID + "^"};
								$.formValidatorBeta.showMessage(returnObj);
							}
						}
					}
				}
		...
		if(isValid)
		{
            if(!initConfigBeta.onSuccess()){return false};
			if(initConfigBeta.submitOnce){$(":submit,:button,:reset").attr("disabled",true);}
		}
		else
		{
			initConfigBeta.onError(firstErrorMessage, $("#" + thefirstid).get(0), errorlist);
			if (thefirstid && initConfigBeta.errorFocus) {$("#" + thefirstid).focus()};
		}
		initConfigBeta.status="init";
		if(isValid && initConfigBeta.debug){alert("现在正处于调试模式(debug:true)，不能提交");}
		return !initConfigBeta.debug && isValid;
	},
	...
	ajaxForm : function(validatorGroup,option,formid)
	{
		...
		var initConfigBeta = $.formValidatorBeta.getInitConfig(validatorGroup);	
		if(formid==undefined){
			formid = initConfigBeta.formID;
			if(formid==""){alert('表单ID未传入');return false;};
		};
		if(!$.formValidatorBeta.pageIsValid(validatorGroup)){return false};
		var ls_url = setting.url;
		var parm = $.formValidatorBeta.serialize('#'+formid);
	...
	serialize : function(objs,initConfigBeta)
	{
		if(initConfigBeta!=undefined){$.formValidatorBeta.resetInputValue(true,initConfigBeta)};
		var parmString = $(objs).serialize();
		if(initConfigBeta!=undefined){$.formValidatorBeta.resetInputValue(false,initConfigBeta)};
	...
	ajaxValid : function(returnObj)
	{
	    ...
	    var srcjo = $("#"+id);
		var elem = srcjo.get(0);
		var initConfigBeta = returnObj.initConfigBeta;
		var settings = elem.settings;
		...
		var initConfigBeta = $.formValidatorBeta.getInitConfig(validatorGroup);
		var parm = $.formValidatorBeta.serialize(initConfigBeta.ajaxObjects);
		//添加触发的控件名、随机数、传递的参数
		...
		{	
			...
			success : function(data, textStatus, jqXHR){
				...
				$.formValidatorBeta.dealAjaxRequestCount(validatorGroup,-1);
				...
				if((typeof lb_ret)=="string")
				{
					...
					ls_status = "onError";
					ls_msg = $.formValidatorBeta.getStatusText(elem,setting.onError);
					//alert(ls_msg)
				}
				setting.isValid = lb_isValid;
				$.formValidatorBeta.setTipState(elem,ls_status,ls_msg);
				//提交的时候触发了ajax校验，等ajax校验完成，无条件重新校验
				if(returnObj.initConfigBeta.status=="sumbitingWithAjax" && returnObj.initConfigBeta.ajaxCountSubmit == 0)
				{
					if (initConfigBeta.formID != "") {
						$('#' + initConfigBeta.formID).trigger('submit');
					}
				}
			},
			complete : function(jqXHR, textStatus){
			...
			beforeSend : function(jqXHR, configs){
				...
				{
					isValid = true;
					$.formValidatorBeta.setTipState(elem,"onLoad",settings[returnObj.ajax].onWait);
				}else
				{
					isValid = false;
					$.formValidatorBeta.setTipState(elem,"onError",lb_ret);
				}
				setting.lastValid = "-1";
				if(isValid){$.formValidatorBeta.dealAjaxRequestCount(validatorGroup,1);}
				return isValid;
			}, 
			error : function(jqXHR, textStatus, errorThrown){
				$.formValidatorBeta.dealAjaxRequestCount(validatorGroup,-1);
			    $.formValidatorBeta.setTipState(elem,"onError",$.formValidatorBeta.getStatusText(elem,setting.onError));
			    setting.isValid = false;
	...
	{
		var initConfigBeta = $.formValidatorBeta.getInitConfig(validatorGroup);
		initConfigBeta.ajaxCountValid = initConfigBeta.ajaxCountValid + val;
		if (initConfigBeta.status == "sumbitingWithAjax") {
			initConfigBeta.ajaxCountSubmit = initConfigBeta.ajaxCountSubmit + val;
		}
	},
	...
	regexValid : function(returnObj)
	{
		...
		else 
		{
			...
			$.each(regexArray, function() {
			    var r = this;
			    if(setting.dataType=="enum"){r = eval("regexEnumBeta."+r);}			
			    if(r==undefined || r=="") 
			    {
			        return false;
			    }
	...
	inputValid : function(returnObj)
	{
		...
		var val = srcjo.val();
		var sType = elem.type;
		var len = $.formValidatorBeta.getLength(id);
	...
	reloadAutoTip : function(validatorGroup)
	{
		...
		$.each(initConfigBeta.validObjects,function()
		{
			if(initConfigBeta.mode == "AutoTip")
			{
				//获取层的ID、相对定位控件的ID和坐标
				var setting = this.settings[0];
	...
	resetInputValue : function(real,initConfigBeta,id)
	{
		var showTextObjects;
		if(id){
			showTextObjects = $("#"+id);
		}else{
			showTextObjects = $(initConfigBeta.showTextObjects);
		}
...
//每个校验控件必须初始化的
$.fn.formValidatorBeta = function(cs) 
{
	cs = cs || {};
	var setting = {};
	...
	var initConfigBeta = $.formValidatorBeta.getInitConfig(cs.validatorGroup);
	
	//校验索引号，和总记录数
	initConfigBeta.validCount += 1;
	
	//如果为精简模式，tipCss要重新设置初始值
	if(initConfigBeta.mode == "SingleTip"){setting.tipCss = {left:10,top:1,width:22,height:22,display:"none"}};
	
	//弹出消息提示模式，自动修复错误
	if(initConfigBeta.mode == "AlertTip"){setting.autoModify=true};
	...
	{
		//记录该控件的校验顺序号和校验组号
		this.validatorIndex = initConfigBeta.validCount - 1;
		...
		if(initConfigBeta.mode == "AutoTip" || initConfigBeta.mode == "FixTip")
		{			
			var tipDiv = $("#"+tip);
			//获取层的ID、相对定位控件的ID和坐标
			if(initConfigBeta.mode == "AutoTip" && tipDiv.length==0)
			{		
		...
		}else if(initConfigBeta.mode == "SingleTip"){
			jqobj.showTooltips();
		}
		...
		$.formValidatorBeta.appendValid(id,setting);

		//保存控件ID
		var validIndex = $.inArray(jqobj,initConfigBeta.validObjects);
		if(validIndex == -1)
		{
			if (setting_temp.ajax) {
				var ajax = initConfigBeta.ajaxObjects;
				initConfigBeta.ajaxObjects = ajax + (ajax != "" ? ",#" : "#") + id;
			}
			initConfigBeta.validObjects.push(this);
		}else{
			initConfigBeta.validObjects[validIndex] = this;
		}

		//初始化显示信息
		if(initConfigBeta.mode != "AlertTip"){
			$.formValidatorBeta.setTipState(this,"onShow",setting.onShow);
		}
		...
		if(srcTag == "input" || srcTag=="textarea")
		{
			if (isInputControl) {
				if(showText !=""){
					showObjs = initConfigBeta.showTextObjects;
					initConfigBeta.showTextObjects = showObjs + (showObjs != "" ? ",#" : "#") + id;
					jqobj.val(showText);
			...
			{	
				...
				if(initConfigBeta.mode != "AlertTip"){
					...
					$.formValidatorBeta.setTipState(this,"onFocus",setting.onFocus);
				};
		    ...
		    jqobj.bind("keyup",function(){
		        ...
		        {
					try{
						var returnObj = $.formValidatorBeta.oneIsValid(id);
						var level = $.formValidatorBeta.passwordValid(this)
						if(level < 0){level=0};
			...
			jqobj.bind(setting.triggerEvent, function(){
				...
				if(returnObj.ajax >= 0) 
				{
					$.formValidatorBeta.showAjaxMessage(returnObj);
				}
				else
				{
					var showmsg = $.formValidatorBeta.showMessage(returnObj);
					if(!returnObj.isValid)
					{
						//自动修正错误
						...
						if(auto)
						{
							...
							if(initConfigBeta.mode != "AlertTip"){$.formValidatorBeta.setTipState(this,"onShow",$.formValidatorBeta.getStatusText(this,setting.onCorrect))};
						}
						else
						{
							if(initConfigBeta.forceValid || setting.forceValid){
...
$.fn.inputValidatorBeta = function(controlOptions)
{
	var settings = {};
	$.extend(true, settings, inputValidator_setting, controlOptions || {});
	return this.each(function(){
		$.formValidatorBeta.appendValid(this.id,settings);
	});
};

$.fn.compareValidatorBeta = function(controlOptions)
{
	var settings = {};
...
$.fn.regexValidatorBeta = function(controlOptions)
{
	var settings = {};
...
$.fn.functionValidatorBeta = function(controlOptions)
{
	var settings = {};
...
$.fn.ajaxValidatorBeta = function(controlOptions)
{
	var settings = {};
	$.extend(true, settings, ajaxValidator_setting, controlOptions || {});
	return this.each(function()
	{
		var initConfigBeta = $.formValidatorBeta.getInitConfig(this.validatorGroup);
		var ajax = initConfigBeta.ajaxObjects;
		if((ajax+",").indexOf("#"+this.id+",") == -1)
		{
			initConfigBeta.ajaxObjects = ajax + (ajax != "" ? ",#" : "#") + this.id;
		}
		this.validatorAjaxIndex = $.formValidatorBeta.appendValid(this.id,settings);
	});
};

$.fn.passwordValidatorBeta = function(controlOptions)
{
	//默认配置
	var settings = {};
	...
	return this.each(function()
	{
	    this.validatorPasswordIndex = $.formValidatorBeta.appendValid(this.id,settings);
	});
};

//指定控件显示通过或不通过样式
$.fn.defaultPassedBeta = function(onShow)
{
	return this.each(function()
	{
		var settings = this.settings;
		...
		{   
			...
			if(!$.formValidatorBeta.getInitConfig(settings[0].validatorGroup).mode == "AlertTip"){
				var ls_style = onShow ? "onShow" : "onCorrect";
				$.formValidatorBeta.setTipState(this,ls_style,settings[0].onCorrect);
			}
		}
	});
};
...
//指定控件不参加校验
$.fn.unFormValidatorBeta = function(unbind)
{
	return this.each(function()
	{
	    if(this.settings)
	...
	return this.each(function()
	{
		...
		jqobj.bind({
			mouseover : function(e){
				...
				$.formValidatorBeta.localTooltip(e);
			},
			mouseout : function(){
				$("#fvtt").hide();
			},
...
var formValidator_setting = 
{
	...
	bind : true,
	ajax : false,
	validateType : "formValidatorBeta",
	tipCss : 
	{
		left:10,
		top:-4,
...
var inputValidator_setting = 
{
	...
	max : 99999,
	onError:"输入错误",
	validateType:"inputValidatorBeta",
...
var compareValidator_setting = 
{
	...
	operateor :"=",
	onError:"输入错误",
	validateType:"compareValidatorBeta"
};
...
var regexValidator_setting = 
{
	...
	compareType : "||",
	onError:"输入的格式不正确",
	validateType:"regexValidatorBeta"
};
...
var ajaxValidator_setting = 
{
	...
	onError:"服务器校验没有通过",
	onWait:"正在等待服务器返回数据",
	validateType:"ajaxValidatorBeta"
};
...
var functionValidator_setting = 
{
	...
	validateType:"functionValidatorBeta",
	onError:"输入错误"
};
...
var passwordValidator_setting = {
	isValid : true,
	compareID : "",
	validateType:"passwordValidatorBeta",
