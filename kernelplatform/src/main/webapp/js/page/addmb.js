$(function() {
        ...
        // 选择用户
        $("#p_checkUser").click(function() { __popUserWindow(); });
        // 选择用户
        $(".vipText").dblclick(function() { __popUserWindow(); });
     ...
     // 保存
        ...
        $("#f_save").click(function() {
           ...
           // alert(scrollTop)
            ...
            if($(".tr_sp").size() < 1) {
            	dialog.init({
            		iconType: 'warn',
            		contentHtml: '运费信息不存在!(本提示5秒后自动关闭)',
            		maskOpacity: 0,
            		autoClose: 5000,
            		yes: function() {
            			dialog.close();
        ...
        $('.onBlurCheckNum1').blur(function(){
        	//为了去除最后一个.
        	$(this).val($(this).val().replace(/\.$/g,""));
        });
        
        $('.onBlurCheckNum').blur(function(){
        	//为了去除最后一个.
        	$(this).val($(this).val().replace(/\.$/g,""));
        });
        ...
        $('.t_area').click(function(){
            ...
            if(calclateType==1){//固定收费
                $(".tr_sp").each(function(_$_,$_$) {
                    var str = $($_$).find("input:eq(4)").val();
                    if(str=="")
                        $($_$).find("input:eq(4)").val("0.00");
                });          
            }
		...
		$('#calclateType').change(function(){
            ...
            if(calclateType==3){//续重价格收费
                $(".tr_sp").each(function(_$_,$_$) {
                    ...
                    $($_$).find("input:eq(1)").val(1.00);//设置默认首重为1
                    $($_$).find(".addWeightChoice").hide();
                });
            }
