...
**/
	function disableSelect(){
		if($("#checkFlag").prop("checked")==false){
			$("#orderBuyer").prop("disabled",true);
		}else{
			$("#orderBuyer").prop("disabled",false);
		}	
	}
		...
		var upload = function() {
			$('#fill_out_btn').click(function(ev) {
				...
				/*uploadInterval = setInterval(function() {
					if ($('#upload').val() != '') {
						...
						$('#dialog .btn').click(function() {
							...
							//uploadDialog.close();
							var uploadConfirm = confirm("您确认本次导入的订单是【好易购(1234)】吗？")
				...
				var uploadFile = '<form id="orderImport_upload_action" action="orderImport_upload.action" method="post" enctype="multipart/form-data">' +
									...
									'	</p>' +
									'	<p>' +
									'		<input id="checkFlag" type="checkbox" checked="checked" value="" onclick="disableSelect()"/>' +
									'		<lable>订单来源</lable>' +
									'		<select name="clientId" id="orderBuyer">'+
									'			<option value="HAOYIGOU">好易购(HAOYIGOU)</option>'+
									'			<option value="TAOBAO">淘宝(TAOBAO)</option>'+
									'			<option value="JAMY">JAMY(JAMY)</option>'+
									'			<option value="QQSD">QQ速递(QQSD)</option>'+
									'			<option value="UCHAOSHI">优超市(UCHAOSHI)</option>'+
									'			<option value="VANCL">凡客(VANCL)</option>'+
									'			<option value="YIXUN">易迅(YIXUN)</option>'+
									'		</select>'+
									'	</p>' +
									'	<p>' +
									'		<lable><font color="red">(提示：如果您不想指定导入订单的来源，请去掉上面的勾选框！)</font></lable>' +
									'	</p>' +
									'' +
									'' +
									'</div>' +
				...
				uploadDialog.init({
					...
					yes: function() {
						...
						if (val.substring(valLen-4, valLen) != '.csv') {
							...
							var confirmStr = "";
							if($("#checkFlag").prop("checked")==false){//未选中
								confirmStr = "您确定不选择订单所属买家，直接上传订单吗？";
							}else{//选中
								var selectedStr = $("#orderBuyer").find("option:selected").text();
								confirmStr = "您确认本次导入的订单来源于【"+ selectedStr +"】吗？";
							}
							
							var uploadConfirm = confirm(confirmStr);
							if(uploadConfirm == true){
								$('#orderImport_upload_action').trigger('submit');
							}
						}
