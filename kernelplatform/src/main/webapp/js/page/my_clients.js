		...
		$('#add_cust_btn').click(function(ev){
			...
			els.buildBtn.click(function(ev){
				...
				if(flag){
					...
					var _name = els.custName.val().replace(/[ ]/g,"");
		...
		var modifyName = function() {
			...
			$('.save_name').live('click', function(ev) {
				...
				if ($.trim(newNameVal) == '') {
					...
					var pattern = /^([0-9a-zA-z_]{1})([\w]*)$/;
					if(pattern.test(newNameVal) == false){
						showIcon.error(_this.closest('td').find('#new_name_inputTip'), '客户名称格式错误 ');
						return;
					}
