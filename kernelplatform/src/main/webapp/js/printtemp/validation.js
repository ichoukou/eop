...
function applyValidate(oField, confs) {
	...
	if (oField.type=="radio" || oField.type=="checkbox") {
		...
		for (var i = 0; i < radios.length; i++) {
			$j(radios[i]).bind('click',validateEventHandler);
			if (confs) {
		...
		$j(oField).bind('blur',validateEventHandler);
		if (confs) {
...
function __handleVaErrMsg(dom, errMsg) {
	if (errMsg && errMsg.length > 0){
	    ...
	    if(lastChild.length==0){		
		    ...
		    $j("<img id='validate_"+uuid+"' src='"+__ctxPath+"/images/validate/exclamation.gif' border=0 class='x-form-invalid-img'>").appendTo("body");
	    ...
	    if(!$j.isFunction($j(document).PQuickTip)){//防止多次加载QuickTip.js文件
	    	$j(document.createElement('script'))
                .attr("src",  __ctxPath + "/scripts/plugins/quicktip/quicktip.js")
          		.attr("type", "text/javascript").appendTo("head");
            $j(document.createElement('link'))
	            .attr("href",  __ctxPath + "/scripts/plugins/quicktip/themes/default/quicktip.css")
...
var rePhone = new RegExp("^(\d{11})|^((\d{7,8})|(\d{4}|\d{3})-(\d{7,8})|(\d{4}|\d{3})-(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1})|(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1}))$");
...
function validateEqlength(str, minLen) {
	return skipIfEmpty(str) || str.length == minLen;
}
