...
<!-- S 当前页面 JS --> 
<script type="text/javascript" src="${jsPath }/page/edit_subaccount.js?d=${str:getVersion() }"></script> 
<!-- E 当前页面 JS -->
      ...
      <div class="box box_a">
        <div class="box_bd">
			<form action="user!editSubAccount.action" method="post" class="form" id="new_acc_form">
				<input type="hidden" id="pageHid" value="edit"/>
				...
				<input name="user.canChangeToContract" value='<s:property value="user.canChangeToContract"/>' type="hidden">
				<p>
					<label for="acc_id"><span class="req">*</span>登录账号：</label>
					<input type="hidden" id="acc_id" class="input_text"  name="user.userName" value="${user.userName}"/>
						${user.userName}
<!-- 					<span id="acc_idTip"></span> -->
						...
						<c:choose>
							...
							<c:otherwise>
								...
								<label class="check_line" for="acc_type_b">
									<input type="checkbox" id="acc_type_b" <c:if test="${user.userType == '2'}">disabled='disabled'</c:if> <c:if test="${user.userType == '11' || user.userType == '13' || user.userType == '21' || user.userType == '23'}">checked='checked'</c:if> class="input_checkbox T" name="range" value="${yto:getCookie('userType')}1" />
									客服:运单管理、消息管理、辅助管理、账号管理
								</label>
								<label class="check_line" for="acc_type_a">
									<input type="checkbox" id="acc_type_a" <c:if test="${user.userType == '2'}">disabled='disabled'</c:if> <c:if test="${user.userType == '12' || user.userType == '13' || user.userType == '22' || user.userType == '23'}">checked='checked'</c:if> class="input_checkbox T" name="range" value="${yto:getCookie('userType')}2" />
                                ...
                                <label class="check_line" for="lj3">
                                    <input id='lj3' <c:if test="${user.userType == '2'}">checked='checked'</c:if> class="input_checkbox T" name="range"  type="checkbox" value="2" />
                                   	承包区：所有功能
                                </label>   	
                                </c:if>
							</c:otherwise>
				...
				<p>
					...
					<!-- 弹窗 -->
					<!-- <div id="smallWindow" style="display:none">
					</div> -->
				</p>
				</c:if>
				<p>
					<label for="init_psw"><span class="req">*</span>登录密码：</label>
					<input type="password" id="edit_psw" class="input_text" name="user.userPassword" value="******" />
