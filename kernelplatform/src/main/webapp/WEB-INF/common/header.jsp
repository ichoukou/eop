	...
	<div id="nav_v2">
		<div id="main_nav" class="clearfix">
			...
			<div id="main_nav_l">
					...
					<ul class="clearfix">
						<c:choose>
							...
							<c:otherwise>
								...
								<c:if test="${yto:getCookie('userType') == 31}">
								   <li menuFlag="shujuchakan" id="menu_b" data-menu="menu_shujuchakan"><a href="javascript:;"><i></i><strong>数据查看</strong></a></li>
								</c:if>
							</c:otherwise>
						</c:choose>
					</ul>
				...
				<div class="menu_level_2 menu_dzmd" style="display:none;">
					...
					<ul>
					   <c:if test="${yto:getCookie('userType') == 2}">
						...
						<li menuFlag="dzmd_mdxz"><a href="waybillExport_userInfoLoad.action?currentPage=1&menuFlag=dzmd_mdxz" title="面单下载">面单下载</a></li>
				        </c:if>
					</ul>
				</div>
				<!-- E 电子面单子菜单 -->
				...
				<!-- S 数据查看子菜单 -->				
				<div class="menu_level_2 menu_shujuchakan" style="display:none;">
					<i class="menu_arrow"></i>
					<ul>
					   <c:if test="${yto:getCookie('userType') == 31}">
						<li menuFlag="shujuchakan_timeLog"><a href="timelog_list.action?menuFlag=shujuchakan_timeLog" title="TimeLog">TimeLog</a></li>
						<li menuFlag="shujuchakan_ordercommand"><a href="ordercommand_list.action?menuFlag=shujuchakan_ordercommand" title="Command">Command</a></li>
				        </c:if>
					</ul>
				</div>
				<!-- E 数据查看子菜单 -->
				...
				<c:if test="${yto:getCookie('userType') == 3}">
					<div class="menu_level_2 menu_c" style="display:none;">
						...
						<ul>
							...
							<li menuFlag="huiyuan_customerAuthority">
								<a href="user!queryUserThread.action?currentPage=1&user.userType=-1&user.userState=-1&queryUserCondition.isCanDownload=-1&menuFlag=huiyuan_customerAuthority" title="面单下载授权">面单下载授权</a>
							</li>
						</ul>
					</div>
				</c:if>
			...
			<ul class="clearfix">
				...
				<c:if test="${str:startsWith(menuFlag,'home') || str:isEmpty(menuFlag) || 
					!(str:startsWith(menuFlag,'chajian_') || str:startsWith(menuFlag,'caiwu_') ||
					 ...
					 str:startsWith(menuFlag,'dzmd_') || str:startsWith(menuFlag,'shujuchakan_'))}">
...
<!-- 			        	    <li menuFlag="caiwu_codsearch" ><a href="codBill_toCompositiveSearch.action?menuFlag=caiwu_codsearch">代收货款综合查询</a></li> -->
				...
				<!-- 数据查看菜单S -->
				<c:if test="${str:startsWith(menuFlag,'shujuchakan_') }">
					<c:if test="${yto:getCookie('userType') == 31}">
					<li menuFlag="shujuchakan_timeLog"><a href="timelog_list.action?menuFlag=shujuchakan_timeLog" title="TimeLog">TimeLog</a></li>
					<li menuFlag="shujuchakan_ordercommand"><a href="ordercommand_list.action?menuFlag=shujuchakan_ordercommand" title="Command">Command</a></li>
					</c:if>
