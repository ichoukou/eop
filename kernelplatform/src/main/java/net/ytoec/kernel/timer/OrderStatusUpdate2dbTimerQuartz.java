...
public class OrderStatusUpdate2dbTimerQuartz extends QuartzJobBean {
	...
	protected void executeInternal(JobExecutionContext context)
		...
		if (!isRunning) {
			try {
				isRunning = true;
				Thread.currentThread().setName("thread.orderStatusUpdate2dbTimer");
				...
				// 状态同步要比金刚时间延迟30分钟，防止订单尚未同步
				if (DateUtil.millisecondsInterval(new Date(), startDate) < (600000*3)) {
					logger.error("订单状态同步------->上次同步时间"+startDate);
					isRunning = false;
					return;
				}
				//状态更新时间比新增的要晚至少5分钟
				String insertKey = tomcatId + "__orderCreate_key";
				Date insertTime = DateUtil.valueof(configCodeService.getConfByKey(insertKey).getConfValue(), dateFormat);
				if (DateUtil.millisecondsInterval(insertTime, startDate) < (60000*5)) {
					logger.error("订单状态同步------->订单创建时间"+insertTime+"上次同步时间"+startDate);
					isRunning = false;
					return;
				} 
				...
				logger.error("----开始执行更新订单状态同步----strStarttime:"+strStarttime+" strEndtime:"+strEndtime);
				...
				logger.error("金刚订单更新同步-->共查询到:" + num + "条数据,金刚检索用时:"
						+ (t2 - t1));
				logger.info("===========查询金刚数据，数据循环数据结束=================");
				...
				try {
					...
					timerLog.setOperate(TimerLog.OPERATE_2);
					timerLog.setTimerNO(tomcatId+"_"+STRING_UPDATE);
					...
					if(errorList.size() > 0){
						throw new RuntimeException("易通订单状态同步------------>异常");
					}
					
					// 定时器下次查询时间更新到数据库
					setNotify();
					timerLog.setIsError(TimerLog.ISERROR_TRUE);
					timerLog.setMessage("批量修改订单状态同步成功");
					logger.error("金刚订单更新同步-->共查询到:" + num + "条数据,插入易通库用时:"
				...
				timerLog.setIsError(TimerLog.ISERROR_FALSE);
				
				// 设置错误日志消息
				timerLog.setMessage("tomcatId为"+tomcatId +
						"订单状态同步Timer出现异常！异常类型：" + e.getMessage() + "异常行数："
						+ ex.getLineNumber());
				Mail mail = new Mail();
				...
				logger.error("订单状态同步主线程finally");
				if(timerLog != null){
					logger.error("订单状态同步-----------> 添加timer日志");
					// 添加日志
		...
		public void run() {
			try {
				...
				for (UpdateInfo orderUpdateInfo : sendUpdateOrderList) {
					...
					try {
						orderService
								.orderStatusNodifyByPartition(updateWaybillInfo);
					} catch (Exception e) {
						try {
							throw e;
						} catch (Exception e1) {
							logger.error(
									"SendOrderStatusUpdate2dbThread 子线程发送异常时，未抛出异常到主线程",
									e1);
							throw e1;
						}
					}
					...
					// 消息值设置
					String message = getObjectFromJson(orderUpdateInfo);
					if (StringUtils.isNotBlank(message)) {
						try {
							
							// logger.error("金刚订单更新同步到MQ物流号："+orderUpdateInfo.getTxLogisticID());
							if (mqService
									.send(message,
					...
					// orderService.orderStatusNodifyAndIntoSolr(updateWaybillInfo);
	...
	public  String getObjectFromJson(UpdateInfo orderUpdateInfo) {
		try
		{
			 BuildSearchStatusWeightIndex bean = orderService.getSolarDto(orderUpdateInfo.getTxLogisticID());
			  if(bean != null){
				 String jsonString = JSON.toJSONString(bean);
				 return jsonString;
			  }
		}catch(Exception e){
			Log.error("OrderStatusUpdate2dbTimerQuartz getObjectFromJson error ",e);
		}
		return null;
	}
