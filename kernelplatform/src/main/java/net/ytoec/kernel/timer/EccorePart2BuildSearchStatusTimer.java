...
/**
 * @author liucy 2012-02-14
 */
@SuppressWarnings("all")
public class EccorePart2BuildSearchStatusTimer extends QuartzJobBean {
	...
	protected void executeInternal(JobExecutionContext arg0)
		...
		try {
			...
			isRunning = true;
			logger.error("EccorePart2BuildSearchStatusTimer start...");

			final CountDownLatch cdl = new CountDownLatch(threadNum);
			
			//TODO  获取订单更新结果集
			List<BuildSearchStatusWeightIndex> list = eccoreSearchService
					.getUpdateBuildData2(limit,2);
			...
			if (allNumber < 1000 && allNumber > 0) {
				logger.error("EccorePart2BuildSearchStatusTimer size <1000");
				
				// TODO MAPPER映射的实体类要与BuildSearchStatusWeightIndex一致
				eccoreSearchService.buildUpdatePartEccoreData(ConfigUtilSingle
						.getInstance().getSolrEccoreUrl(), list,"2");
				
				isRunning = false;
				return;
			} else if (allNumber <= 0) {
				logger.error("EccorePart2BuildSearchStatusTimer size <=0");
				isRunning = false;
				return;
			}
			int subNumber=0;
			...
			for (int d = 0; d < threads.length; d++) {
				...
				// 设置线程名称
				synWeightThread.setName("statussub2_" + d);
			...
			cdl.await();*/
			list.clear();
			logger.error("EccorePart2BuildSearchStatusTimer end...");
			isRunning = false;
		...
		public void run() {
			try {
				...
				if (buildSearchStatusWeightIndexs.size() > 0) {
					eccoreSearchService.buildUpdatePartEccoreData(ConfigUtilSingle.getInstance().getSolrEccoreUrl(),buildSearchStatusWeightIndexs,"2");
	...
	public void setLimit(int limit) {
		EccorePart2BuildSearchStatusTimer.limit = limit;
	}
	...
	public void setThreadNum(int threadNum) {
		EccorePart2BuildSearchStatusTimer.threadNum = threadNum;
	}
}
