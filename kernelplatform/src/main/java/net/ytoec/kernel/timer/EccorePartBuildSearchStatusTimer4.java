...
public class EccorePartBuildSearchStatusTimer4 extends QuartzJobBean {
	...
	protected void executeInternal(JobExecutionContext arg0)
		...
		try {
			...
			isRunning = true;
			logger.error("EccorePartBuildSearchStatusTimer4 start...");

			final CountDownLatch cdl = new CountDownLatch(threadNum);
			
			//TODO  获取订单更新结果集
			List<BuildSearchStatusWeightIndex> list = eccoreSearchService
					.getUpdateBuildData2(limit,5);
			...
			if (allNumber < 1000 && allNumber > 0) {
				logger.error("EccorePartBuildSearchStatusTimer4 size <1000");
				
				// TODO MAPPER映射的实体类要与BuildSearchStatusWeightIndex一致
				eccoreSearchService.buildUpdatePartEccoreData(ConfigUtilSingle
						.getInstance().getSolrEccoreUrl(), list,"5");
				
				isRunning = false;
				return;
			} else if (allNumber <= 0) {
				logger.error("EccorePartBuildSearchStatusTimer4 size <=0");
				isRunning = false;
				return;
			}
			int subNumber=0;
			...
			for (int d = 0; d < threads.length; d++) {
				...
				// 设置线程名称
				synWeightThread.setName("statussub4_" + d);
				threads[d] = synWeightThread;
				logger.error("thread领取subBuildSearchStatus4_" + d + ",size:"
			...
			cdl.await();*/
			list.clear();
			logger.error("EccorePartBuildSearchStatusTimer4 end...");
			isRunning = false;
			...
			Mail mail = new Mail();
			mail.setSubject("EccorePartBuildSearchStatusTimer4出现异常！");
		...
		public void run() {
			try {
				...
				if (buildSearchStatusWeightIndexs.size() > 0) {
					eccoreSearchService.buildUpdatePartEccoreData(ConfigUtilSingle.getInstance().getSolrEccoreUrl(),buildSearchStatusWeightIndexs,"5");
					 
				}
			} catch (Exception e) {
				logger.error("BuildSearchStatusThread4 error", e);
			} finally {
				// 线程结束时计数器减1
	...
	public void setLimit(int limit) {
		EccorePartBuildSearchStatusTimer4.limit = limit;
	}
	...
	public void setThreadNum(int threadNum) {
		EccorePartBuildSearchStatusTimer4.threadNum = threadNum;
	}
}
