...
public class EccorePartBuildSearchStatusTimer2 extends QuartzJobBean {
	...
	protected void executeInternal(JobExecutionContext arg0)
		...
		try {
			...
			isRunning = true;
			logger.error("EccorePartBuildSearchStatusTimer2 start...");
			...
			List<BuildSearchStatusWeightIndex> list = eccoreSearchService
					.getUpdateBuildData2(limit,3);
			logger.error("mylogt3:getdata"+(System.currentTimeMillis()-t1));
			// 结果集为空的场合
			if (list == null) {
				isRunning = false;
				return;
			...
			if (allNumber < 1000 && allNumber > 0) {
				logger.error("EccorePartBuildSearchStatusTimer2 size <1000");
				
				// TODO MAPPER映射的实体类要与BuildSearchStatusWeightIndex一致
				eccoreSearchService.buildUpdatePartEccoreData(ConfigUtilSingle
						.getInstance().getSolrEccoreUrl(), list,"3");
				
				isRunning = false;
				return;
			} else if (allNumber <= 0) {
				logger.error("EccorePartBuildSearchStatusTimer2 size <=0");
				isRunning = false;
				return;
			}
			int subNumber=0;
            ...
            logger.error("mylogt3:pushdata"+(System.currentTimeMillis()-t2));
			...
			for (int d = 0; d < threads.length; d++) {
				...
				// 设置线程名称
				synWeightThread.setName("statussub22_" + d);
				threads[d] = synWeightThread;
				logger.error("thread领取subBuildSearchStatus2_" + d + ",size:"
			...
			cdl.await();*/
			list.clear();
			logger.error("EccorePartBuildSearchStatusTimer2 end...");
			isRunning = false;
			...
			Mail mail = new Mail();
			mail.setSubject("EccorePartBuildSearchStatusTimer2出现异常！");
		...
		public void run() {
			try {
				...
				if (buildSearchStatusWeightIndexs.size() > 0) {
					eccoreSearchService.buildUpdatePartEccoreData(ConfigUtilSingle.getInstance().getSolrEccoreUrl(),buildSearchStatusWeightIndexs,"3");
					 
				}
			} catch (Exception e) {
				logger.error("BuildSearchStatusThread2 error", e);
			} finally {
				// 线程结束时计数器减1
	...
	public void setLimit(int limit) {
		EccorePartBuildSearchStatusTimer2.limit = limit;
	}
	...
	public void setThreadNum(int threadNum) {
		EccorePartBuildSearchStatusTimer2.threadNum = threadNum;
	}
}
