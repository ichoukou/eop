...
public class EccorePart2BuildSearchTimer extends QuartzJobBean {

    private EccoreSearchService             eccoreSearchService;

    private static int                      limit  = 1000;
    private static String                   buildFlag  = "1";

    private static boolean                  isRunning = false;

    private static Logger                   logger = LoggerFactory.getLogger(EccorePart2BuildSearchTimer.class);
    ...
    protected void executeInternal(JobExecutionContext arg0) throws JobExecutionException {
        try {
            ...
            List<BuildSearch> list=eccoreSearchService.getBuildForAddData(limit);
            if(list == null){
            	isRunning = false;
            	return;
            }
            ...
            if(list.size()<1000&&list.size()>0){
            	...
            	eccoreSearchService.newBuildPartEccoreData(ConfigUtilSingle.getInstance().getSolrEccoreUrl(),list,buildFlag);
            	isRunning = false;
            	return;
            }else if(list.size()<=0){
            ...
            for(int i=0;i<threadNum;i++){
	            ...
	            MyThread myThread = new MyThread(){
					...
					public void run() {
						try{
							eccoreSearchService.newBuildPartEccoreData(ConfigUtilSingle.getInstance().getSolrEccoreUrl(),sublist,buildFlag);
    ...
    public void setLimit(int limit) {
        EccorePart2BuildSearchTimer.limit = limit;
    }
	...
	public void setThreadNum(int threadNum) {
		EccorePart2BuildSearchTimer.threadNum = threadNum;
	}
}
