...
public class ZebraSurfacebillServiceImpl implements ZebraSurfacebillService {
	...
	public List<UserWaybillInfo> countWaybillInfo(List<String> customerCodeList){
		return zebraSurfacebillDao.countWaybillInfo(customerCodeList);
	}
	...
	public HSSFWorkbook exportWaybills(ZebraSequence zebraSequence,String customerName)throws Exception{
		...
		boolean isNeedUnlock = false;
			// 如果有传入sequenceId，则为重复拉取面单
			if(zebraSequence.getSequenceId() == null || "".equals(zebraSequence.getSequenceId())){
				// 控制并发
				lock.setBusinessId(zebraSequence.getCustomerCode());
				if(!syncLockDao.isExistsSyncLock(lock)){
					syncLockDao.insertSyncLock(lock);
				}else{
					if(syncLockDao.updateByLockState(lock) <= 0){
						throw new BusinessException(Constant.MSG_DOWNLOAD_FAST);
					}
				}
				isNeedUnlock = true;
				// 保存sequence记录
				...
				map.put("customerCode", zebraSequence.getCustomerCode());
				map.put("quantity",zebraSequence.getTotalCount());
				map.put("sequence", zebraSequence.getSequenceId());
				zebraSurfacebillDao.batchUpdateStatebyQuantity(zebraSequence.getCustomerCode(),
						zebraSequence.getTotalCount(),zebraSequence.getSequenceId().toString());
				// 解除锁定
				if(isNeedUnlock){
					lock.setLock(false);
			...
			waybillNoList = 
				zebraSurfacebillDao.queryWaybillBySequence(zebraSequence.getCustomerCode(), zebraSequence.getSequenceId());
			// 如果数量与客户想要的不一致，则终止导出
			if(waybillNoList.size() != zebraSequence.getTotalCount()){
				throw new RuntimeException(Constant.MSG_WAYBILL_NOTENOUGH);
			}
			// 封装成EXCEL
			HSSFSheet sheet = hSSFWorkbook.createSheet(zebraSequence.getSequenceId().toString());
			...
			cell.setCellValue("面单号");
			cell.setCellStyle(style);
			cell = row.createCell(2);
			cell.setCellValue("商家代码");
			cell.setCellStyle(style);
			...
			for (int i = 0; i < waybillNoList.size(); i++){
				...
				row.createCell(0).setCellValue(zebraSequence.getSequenceId());
				row.createCell(1).setCellValue(waybillNoList.get(i));
				row.createCell(2).setCellValue(zebraSequence.getCustomerCode());
