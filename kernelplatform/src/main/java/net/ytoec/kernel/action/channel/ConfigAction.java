 ...
 * 
 * @author Wangyong
 * @2011-8-29 net.ytoec.kernel.action.channel
 */
@Component
@Scope("prototype")
public class ConfigAction extends AbstractActionSupport {
	...
	public String index() {
		...
		if (currentUser.getUserType() != null
				&& currentUser.getUserType().equals("3")) {
			configList = configService.getConfigByLevel(pagination, "1", true);
			pagination.setTotalRecords(configService.getConfigByLevel(
					pagination, "1", false).size());
		} else {
