...
public class YtoUserAction extends AbstractActionSupport {
	...
	private Logger logger = LoggerFactory.getLogger(YtoUserAction.class);
...
//		Integer total = userService.getAllUserMap(params, pagination, false).size();
//		pagination.setTotalPages(total);
		
		
		if(StringUtil.isBlank(user.getUserName())|| (StringUtils.isNotBlank(startTime) && StringUtils.isNotBlank(endTime))){
			pagination.setTotalRecords(userService.searchTotal(params));
		}
	...
	public String addYtoUserAccount() throws IOException{
		...
		}else if(user.getUserType().equals("2") || user.getUserType().equals("4")){
			user.setUserState("TBA");
			user.setUserCode(user.getUserName());
			user.setTaobaoEncodeKey(user.getUserName());
