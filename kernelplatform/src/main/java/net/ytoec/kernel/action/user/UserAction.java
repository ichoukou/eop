...
public class UserAction extends AbstractActionSupport {
	...
	private Pagination<UserThread> userPagination;

	public Pagination<UserThread> getUserPagination() {
		return userPagination;
	}

	public void setUserPagination(Pagination<UserThread> userPagination) {
	 ...
	 * @return
	 */
	public String queryUserThread() {
		if(queryUserCondition==null){
			queryUserCondition = new QueryUserCondition();
		}

		queryUserCondition.setStartLine((currentPage-1)*pageNum);
		queryUserCondition.setLimit(pageNum);
		
//		/**对网点代码格式验证，若网点代码不为空，且不符合规则，则不进行查询直接返回*/
//		boolean boo = true;
//		if(queryUserCondition.getSiteCode()!=null && !queryUserCondition.getSiteCode().equals("")&& !queryUserCondition.getSiteCode().matches("^\\d+$")){
//			boo = false;
//		}
//		/**对商家代码格式验证，若商家代码不为空，且不符合规则，则不进行查询直接返回*/
//		if(queryUserCondition.getCustomerCode()!=null && !queryUserCondition.getCustomerCode().equals("") && !queryUserCondition.getCustomerCode().matches("^[0-9a-zA-Z]*$")){
//			boo = false;
//		}
//		
//		if(boo){
			userThreadList = userThreadService.queryUserThread(queryUserCondition);
//		}
		userPagination = new Pagination(currentPage, pageNum);
		if (userThreadList == null || userThreadList.size() == 0) {
			userPagination.setTotalRecords(0);
		}else{
			userPagination.setTotalRecords(userThreadService.countUserThread(queryUserCondition));
		}
	 ...
	 * @return
	 */
	public String updateIsCanDownload() {
		List<String> list = null;
		boolean result = false;
		if(customerIDs!= null && !customerIDs.equals("")){
			list = Arrays.asList(customerIDs.split(","));
			result = userThreadService.updateIsCanDownloadByIds(list, isCanDownload);
		}
		
		if(result){
			putMsg(JsonResponse.INFO_TYPE_SUCCESS, true, "授权操作成功！",
			"user!queryUserThread.action?currentPage=1&user.userType=-1&user.userState=-1");
		}else{
			putMsg(JsonResponse.INFO_TYPE_ERROR, false, "操作失败，请重新操作！",
					"user!queryUserThread.action?currentPage=1&user.userType=-1&user.userState=-1");
		}
